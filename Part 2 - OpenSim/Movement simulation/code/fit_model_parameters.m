...
vmax = 5;
N = 1000;
[vts, Fts, toc, idF, idFd] = design_length_input_vector(vmax, N);
dt1 = mean(diff(toc));

%% specify some parameters (need to just load different .mat)
% Horslen parameters
...
%% specify parameters that will be optimized
% parameters
...
% simulate response to length input
parms.vts = vts;
parms.ti = toc;

sol = ode15i(@(t,y,yp) fiber_dynamics_implicit_no_tendon(t,y,yp, parms), [0 max(toc)], X0, xp0);
t = sol.x;
x = sol.y;
[~,xdot] = deval(sol, t);
...
% dynamic constraints (need to simplify)
Ns = [0 N];
id = (Ns(1)+1):Ns(end);
F = (Q0(id) + Q1(id));

% make sure things can't be negative (need to also implement in simulation)
k = 20;
...
%% Test protocol
osol = ode15i(@(t,y,yp) fiber_dynamics_implicit_no_tendon(t,y,yp, parms), [0 max(toc)], X0, xp0);
t = osol.x;
x = osol.y;
F = x(1,:) + x(2,:);
[~,xdot] = deval(osol, t);
